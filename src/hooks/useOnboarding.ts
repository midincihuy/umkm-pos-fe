...
export function useOnboarding(): OnboardingState {
  ...
  // Kalau user sudah punya rekening → dismiss flag tidak relevan, clear it
  if (accountCount > 0 && isDismissed()) clearDismissed()

  let status: OnboardingStatus = 'complete'
  if      (accountCount === 0)                         status = 'no_account'
  ...
  if (dismissed && accountCount === 0) status = 'complete'
